...
function handleAppointment() {
    const doctolibUrl = getUrlParameter('doctolib');
    
    if (doctolibUrl) {
        try {
            const decodedUrl = decodeURIComponent(doctolibUrl);
            window.open(decodedUrl, '_blank');
            showNotification('Ouverture de Doctolib...', 'success');
        } catch (error) {
            console.error('Erreur lors de l\'ouverture de Doctolib:', error);
            showNotification('Erreur lors de l\'ouverture de Doctolib', 'error');
        }
    } else {
        showNotification('URL Doctolib non disponible', 'warning');
    }
}
