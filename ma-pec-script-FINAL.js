// Configuration
const CONFIG = {
    stripeApiUrl: 'https://api.stripe.com/v1/payment_intents',
    sspApiUrl: 'https://api.veasy.com/v1/payments',
    financementUrl: 'https://financement.cemedis.app'
};
...
function populateFinancialInfo() {
    ...
    document.getElementById('total-amount-financement').textContent = `${total.toFixed(2)} €`;
}
...
function setupTreatmentTabs() {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            const treatmentType = this.getAttribute('onclick').match(/'([^']+)'/)[1];
...
function showTreatmentInfo(treatmentType) {
    ...
    const activeButton = document.querySelector(`[onclick="showTreatmentInfo('${treatmentType}')"]`);
    if (activeButton) {
    ...
    // Afficher l'information de traitement sélectionnée
    const targetInfo = document.getElementById(`${treatmentType}-info`);
    if (targetInfo) {
